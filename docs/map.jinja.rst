...
We will see a quick setup to configure the ``pgpool`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``pgpool`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``pgpool`` formulas under ``/srv/salt/pgpool/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/pgpool/parameters/dns:domain/
    ln -s dns:domain /srv/salt/pgpool/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/pgpool/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/pgpool/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``pgpool`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``pgpool`` formulas under ``/srv/salt/pgpool/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/pgpool/parameters/roles
...
We create a configuration for the role ``pgpool/server`` in ``/srv/salt/pgpool/parameters/roles/pgpool/server.yaml``:
...
We create another configuration for the role ``pgpool/client`` in ``/srv/salt/pgpool/parameters/roles/pgpool/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles pgpool/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles pgpool/client
...
.. code-block:: console
    ...
    mv /srv/salt/pgpool/parameters/dns:domain/  '/srv/salt/pgpool/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-pgpool-as-json:
      test.nop:
        - name: {{ pgpool | json }}
...
Here is an example based on `template-formula/pgpool/config/file.sls`_:
...
This ``sls`` file expose a ``pgpool`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ pgpool['winner'] }}
...
.. _template-formula/pgpool/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/pgpool/config/file.sls
